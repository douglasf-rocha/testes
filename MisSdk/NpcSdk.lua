...
dofile(GetResPath("ctf/Functions.lua"))
------------------------------------------------------------
...
function Sale( character, npc, index, count )
	...
	if KitbagLock(character, 1) == LUA_TRUE or GetChaStateLv(character, STATE_BAT) >= 1 or GetChaStateLv(character, STATE_JY) >= 1 then
        SystemNotice(character,"Your Inventory is locked!")
        return LUA_FALSE
    end	
...
function MsgProc( character, npc, rpk, page, trade, missionlist, exchangeData )
	...
	elseif usCmd == CMD_CM_FUNCITEM then --功能页选项命令信息处理
		...
		elseif item.func == CTF.DisplayInfo then
		print("display info")
			return CTF.DisplayInfo( character )
