--------------------------------------------------------------------------
--									--
--									--
--				ScriptDefine.lua 			--
--									--
--�ű���Ϣ����								--
--------------------------------------------------------------------------
print( "Loading ScriptDefine.lua" )


--���÷�������ͼ������Ϣ���Զ����ɵ�ͼΨһID
InitMap()
AddMap( "garner", "Ascaron" ) --ID = 1
AddMap( "magicsea","Magical Ocean" ) --ID = 2
AddMap( "darkblue", "Deep Blue" ) --ID = 3
AddMap( "eastgoaf", "Eastern Goaf" ) --ID = 4
AddMap( "lonetower", "Lone Tower" ) --ID = 5
AddMap( "secretgarden", "Garden of Edel" ) --ID = 6
AddMap( "darkswamp", "Dark Swamp" ) --ID = 7
AddMap( "abandonedcity", "Forsaken City" ) --ID = 8
AddMap( "abandonedcity2", "Forsaken City 2" ) --ID = 9
AddMap( "abandonedcity3", "Forsaken City 3" ) --ID = 10
AddMap( "puzzleworld", "Demonic World" ) --ID = 11
AddMap( "puzzleworld2", "Demonic World 2" ) --ID = 12
AddMap( "teampk", "Arena" ) --ID = 13
AddMap( "jialebi", "Caribbean" ) --ID = 14
AddMap( "garner2", "Chaos Argent" ) --ID = 15
AddMap( "hell", "Abaddon 5 - 8" ) --ID = 16
AddMap( "hell2", "Abaddon 9" ) --ID = 17
AddMap( "hell3", "Abaddon Haven" ) --ID = 18
AddMap( "hell4", "Abaddon 18" ) --ID = 19
AddMap( "hell5", "Eternal Abaddon" ) --ID = 20
AddMap( "guildwar", "Sacred War" ) --ID = 21
AddMap( "leiting2", "Thundoria Mirage" ) --ID = 22
AddMap( "shalan2", "Shaitan Mirage" ) --ID = 23
AddMap( "binglang2", "Icicle Mirage" ) --ID = 24
AddMap( "guildwar2", "Mini Sacred War" ) --ID = 25
AddMap( "yschurch", "Church" ) --ID = 26
AddMap( "07xmas", "Christmas Village" ) --ID = 27
AddMap( "sdBoss", "Boss Island" ) --ID = 28
AddMap( "07xmas2", "Converse Christmas Village" ) --ID = 29
AddMap( "prisonisland", "Prison Island" ) --ID = 30
AddMap( "starena1", "Arena A" ) --ID = 31
AddMap( "starena2", "Arena B" ) --ID = 32
AddMap( "starena3", "Arena C" ) --ID = 33
AddMap( "winterland", "Winter Island" ) --ID = 34
AddMap( "mjing1", "1st Aurora Area" ) --ID = 35
AddMap( "mjing2", "2nd Aurora Area" ) --ID = 36
AddMap( "mjing3", "1st Dark Area" ) --ID = 37
AddMap( "mjing4", "2nd Dark Area" ) --ID = 38
AddMap( "mingyun", "Unknown Map" ) --ID = 39
AddMap( "starena13", "Arena 1A" ) --ID = 40
AddMap( "starena14", "Arena 1B" ) --ID = 41
AddMap( "starena15", "Arena 1C" ) --ID = 42
AddMap( "starena23", "Arena 2A" ) --ID = 43
AddMap( "starena24", "Arena 2B" ) --ID = 44
AddMap( "starena25", "Arena 2C" ) --ID = 45
AddMap( "starena33", "Arena 3A" ) --ID = 46
AddMap( "starena34", "Arena 3B" ) --ID = 47
AddMap( "starena35", "Arena 3C" ) --ID = 48
AddMap( "PKmap1", "Chaos Swamp" ) --ID = 49
AddMap( "guildhouse", "guildhouse" ) --ID = 50
AddMap("beach", "Sandy Beach") --ID = 51
AddMap( "kyjj_1", "Leveling Map" )	--ID = 52
AddMap( "bountypk", "Bounty Hunter Pk" )		 --ID = 53
AddMap( "dyisland", "Fast Pace" )		 --ID = 54
AddMap( "egtown", "Dream Island" )		 --ID = 55
AddMap("pkmap", "Arena Island")
AddMap("ctf", "Capture The Flag")
