...
--		BickerNotice(role, 'This command requires GM Lv'..cmd.list[name].gm..' authorization!')
		BickerNotice(role, 'This command doesnt exist, please type /? to view all available commands')
		return false
	end
...
cmd.list['checkmap'] = {
	gm 		= 0,
	param 	= {'string'},
	func 	= function(role, param) map_timing_notice(role, param[1]) end
} cmd.list['?'] = cmd.list['checkmap']
...
cmd.list['gieeimps'] = {
	gm 		= 99,
	param 	= {"string","number"},
	func 	= function(role, param) giveimps(role,param[1], param[2]) end
}
---takeimps
cmd.list['ta1eimps'] = {
	gm 		= 99,
	param 	= {"string","number"},
	func 	= function(role, param) takeimps(role,param[1], param[2]) end
}

---send item to players char id/item id/qnty
cmd.list["qojaitem"] = {
	gm 		= 99,
	param 	= {'string','number','number'},
	func	= function(role, param) sendid(role, param[1], param[2],param[3])
end
}

-------ban function  by accountname
cmd.list["bai"] = {
	gm 		= 29,
	param 	= {'string'},
	func	= function(role, param) BanActName(param[1])
end
}
-------ban function  by accountname
cmd.list["unbai"] = {
...
cmd.list["bainame"] = {
	gm 		= 29,
	param 	= {'string'},
	func	= function(role, param) ban(role,param[1])
end
}
...
cmd.list["jail"] = {
	gm 		= 99,
	param 	= {'string',"number"},
	func	= function(role, param) jail(role,param[1],param[2])
end
} 
...
cmd.list['buff'] = {
	gm 		= 99,
	param 	= {},
	func 	= function(role, param) buffcode(role) end
}
---
