print('Loading ChaosArgent.lua')
--[[
INFO:
- fix CA intervals, will always spawn wave at :05, :10, :15, :20, :25, :30, and :35
- records how many unique players and how many total players
- records how many kills each players have

TODO:
- prize handler, easier adjustment to prizes can be done by table
- add all entry money and give to winner(s)
]]
...
chaospk.pos           = { mapName   = 'Icicle City',
          x           = 1370,
          y           = 532,
          map         = 'darkblue' }
chaospk.entryFee      = {
...
chaospk.dataInit = function()
  chaospk.data = chaospk.data or {}
  if chaospk.data['wave'] == nil then
    chaospk.data['wave'] = {}
    for i = 1, 8 do chaospk.data['wave'][i*5] = {num = i, active = false} end
  end
  chaospk.data['cha'] = chaospk.data['cha'] or {}
end

chaospk.release = function() chaospk.data = nil end

chaospk.pvpHook = GetExp_PKP

GetExp_PKP = function(dead, atk)
  chaospk.pvpHook(dead, atk)
  if GetChaMapName(dead) == chaospk.conf['mapName'] and
    GetChaMapName(dead) == GetChaMapName(atk) then
    chaospk.data['cha'][atk].kills = (chaospk.data['cha'][atk].kills + 1) or 0
  end
end

chaospk.getchacount = function()
  local unique, counts = 0,0
  for i,v in pairs(chaospk.data['cha']) do
    unique = unique + 1
    counts = counts + v.entry
  end return unique, counts
end

_G['get_map_entry_pos_'..chaospk.conf['mapName']] = function() return chaospk.pos['x'], chaospk.pos['y'] end

_G['after_enter_'..chaospk.conf['mapName']] = function(role, map_copy)
  chaospk.dataInit()
  chaospk.data['cha'][role] = chaospk.data['cha'][role] or {}
  if chaospk.data['cha'][role].entry == nil then chaospk.data['cha'][role].entry = 1
  else chaospk.data['cha'][role].entry = chaospk.data['cha'][role].entry + 1
  end
  chaospk.data['cha'][role].kills = chaospk.data['cha'][role].kills or 0
  SetPortalName("<Chaos Argent>", string.format("%d player(s) inside", GetMapActivePlayer(map_copy)))
  Notice('Player ['..GetChaDefaultName(role)..'] enters Chaos Argent');
end

_G['before_leave_'..chaospk.conf['mapName']] = function(role) 
  SetPortalName("<Chaos Argent>", string.format("%d player(s) inside", GetMapActivePlayer(GetChaMapCopу(role)) - 1))
end
_G['map_run_'..chaospk.conf['mapName']] = function(map) end

_G['map_copy_before_close_'..chaospk.conf['mapName']] = function(map_copy)
  How_Many_Active = GetMapActivePlayer(map_copy)
  if How_Many_Active > 0 then DealAllActivePlayerInMap(map_copy, "GiveSpecItem") end
  How_Many_Active = 0
  Money_all = 100000
  Every_5_minute = 0
  chaospk.release()
...
_G['map_copy_run_'..chaospk.conf['mapName']] = function(map_copy)
  local m = tonumber(os.date('%M'))
  local h = GetMapActivePlayer(map_copy)
  if chaospk.data['wave'][m] ~= nil and not chaospk.data['wave'][m].active then
    chaospk.spawn(map_copy, chaospk.data['wave'][m].num)
    chaospk.data['wave'][m].active = true
  end
end

_G['map_copy_close_'..chaospk.conf['mapName']] = function(map_copy) ClearAllSubMapCha(map_copy) end
_G['can_open_entry_'..chaospk.conf['mapName']] = function(map) 
	local month = os.date ("%m")
	local day = os.date ("%d")
	local hour = os.date ("%H")
  local hournum = tonumber(hour)
	if hournum == 03 or hournum == 09 or hournum == 15 or hournum == 21 then
		LG('ChaosArgent-'..day..'', ' CA has opened today('..day..'/'..month..') at ('..hour..')')
		return 1
	else
		return 0
	end
end
_G['after_destroy_entry_'..chaospk.conf['mapName']] = function(entry)
  map_name, posx, posy, tmap_name = GetMapEntryPosInfo(entry) 
  ScrollNotice('The portal to '..chaospk.entry['mapName']..' has vanished.', 1) 
end

_G['after_player_login_'..chaospk.conf['mapName']] = function(entry, player_name)
  map_name, posx, posy, tmap_name = GetMapEntryPosInfo(entry)
  ChaNotice(player_name, 'Portal to '..chaospk.entry['mapName']..' has opened at ['..posx..','..posy..'] '..chaospk.pos['mapName'])
end

_G['check_can_enter_'..chaospk.conf['mapName']] = function(role, copy_mgr)
  local FightingBook_Num = 0
  FightingBook_Num = CheckBagItem(role, 3849)
  local Team_In = IsInTeam(role)
  --if Team_In == 1 then
   -- SystemNotice(role , "You are now in a party. Unable to enter "..chaospk.entry['mapName'].."" )
   -- return 0
  --end
  if FightingBook_Num <= 0 then
    SystemNotice(role , "You do not have a Medal of Valor to enter "..chaospk.entry['mapName']..". Please obtain it from Arena Administrator." )
    return 0
  elseif FightingBook_Num > 1 then
    BickerNotice(role, 'You have one too many Medal of Valor. Contact administrator to enter!')
    return 0
  end
	local Num_1= CheckBagItem(role,3098)
    if Num_1 >= 1 then
        SystemNotice(role, "Please, remove [Constitution Recovery Flask] from your inventory!")    
        return 0
  end
	local Num_2= CheckBagItem(role,3099)
    if Num_2 >= 1 then
        SystemNotice(role, "Please, remove [SP Holy Water] from your inventory!")    
        return 0
  end
  local role_RY = GetChaItem2(role, 2, 3849)
  local HonorPoint = GetItemAttr(role_RY, ITEMATTR_VAL_STR)
  if HonorPoint > 30000 then
    SystemNotice ( role , "Your honor is too high, unable to enter "..chaospk.entry['mapName'].."" )
    return 0
  end
  if Lv(role) < chaospk.entryFee['level'] then
    SystemNotice(role, "To enter "..chaospk.entry['mapName']..", players need to be above level "..chaospk.entryFee['level'].."!")
    return 0    
  end
  if HasMoney(role, chaospk.entryFee['gold']) == 1 then
    return 1
  else
    SystemNotice(role,"To enter "..chaospk.entry['mapName']..", players must pay a FEE: "..chaospk.entryFee['gold'].."g")
    return 0
  end    
end

_G['begin_enter_'..chaospk.conf['mapName']] = function(role, copy_mgr)
  TakeMoney(role, nil, chaospk.entryFee['gold']);
  SystemNotice(role, "Entering ["..chaospk.entry['mapName'].."], collects fee: "..chaospk.entryFee['gold'].."g!");
  MoveCity(role, chaospk.entry['mapName']);
end


function GiveSpecItem(role)
  if How_Many_Active == 0 then                ScrollNotice('There were no winners this Chaos Argent!', 1)
                                              return end
  local Money_Garner2 = Money_all / How_Many_Active
  if How_Many_Active > 5 then                 AddMoney(role, 0, Money_Garner2)
                                              return end
  if How_Many_Active <  6 and How_Many_Active > 1 then
    if GetChaFreeBagGridNum(role) <= 0 then   GiveItemX(role, 0, 1031, 1, 4)
    else                                      GiveItem(role, 0, 1031, 1, 4)
    end                                       AddMoney(role, 0, Money_Garner2)
                                              return end
  if How_Many_Active == 1 then
    if GetChaFreeBagGridNum(role) <= 3 then
      GiveItemX(role, 0, 15085, 1, 4)
      GiveItemX(role, 0, 15972, 2, 4)
    else
      GiveItem(role, 0, 15085, 1, 4)
      GiveItem(role, 0, 15972, 2, 4)
    end
    AddMoney(role, 0, 8000000)
    ScrollNotice(chaospk.entry['mapName']..' Last man Standing is: '..GetChaDefaultName(role)..' has obtained [ 1x Random Apparel | 2x Potion Pack | 8m Gold ]!', 2)
		LG('CAWinners', ''..GetChaDefaultName(role)..' is the only survivor in Chaos Argent and obtained [ 1x Random Apparel | 2x Potion Pack | 8m Gold ]')
    return
  end
end 

chaospk.spawn = function(map_copy, wave)
  Every_5_minute = tonumber(wave);
  local h = GetMapActivePlayer(map_copy)
  ScrollNotice('Chaos Argent wave ['..Every_5_minute..'] spawned. Currently '..h..' active player in Chaos Argent', 1)
  ...
  if Every_5_minute == 7 then
    ...
    SetChaLifeTime(Monster168, 50000)
    SetChaLifeTime(Monster169, 50000)
    SetChaLifeTime(Monster170, 50000)
    SetChaLifeTime(Monster171, 50000)
    SetChaLifeTime(Monster172, 50000)
    SetChaLifeTime(Monster173, 50000)
    SetChaLifeTime(Monster174, 50000)
    SetChaLifeTime(Monster175, 50000)
    SetChaLifeTime(Monster176, 50000)
    SetChaLifeTime(Monster177, 50000)
    SetChaLifeTime(Monster178, 50000)
    SetChaLifeTime(Monster179, 50000)
    SetChaLifeTime(Monster180, 50000)
    SetChaLifeTime(Monster181, 50000)
    SetChaLifeTime(Monster182, 50000)
    SetChaLifeTime(Monster183, 50000)
    SetChaLifeTime(Monster184, 50000)
    SetChaLifeTime(Monster185, 50000)
    SetChaLifeTime(Monster186, 50000)
    SetChaLifeTime(Monster187, 50000)
    SetChaLifeTime(Monster188, 50000)
    SetChaLifeTime(Monster189, 50000)
    SetChaLifeTime(Monster190, 50000)
    SetChaLifeTime(Monster191, 50000)
    SetChaLifeTime(Monster192, 50000)
    SetChaLifeTime(Monster193, 50000)
    SetChaLifeTime(Monster194, 50000)
    SetChaLifeTime(Monster195, 50000)
    SetChaLifeTime(Monster196, 50000)
    SetChaLifeTime(Monster197, 50000)
    SetChaLifeTime(Monster198, 50000)
    SetChaLifeTime(Monster199, 50000)
    SetChaLifeTime(Monster200, 50000)
    SetChaLifeTime(Monster201, 50000)
    SetChaLifeTime(Monster202, 50000)
    SetChaLifeTime(Monster203, 50000)
    SetChaLifeTime(Monster204, 50000)
    SetChaLifeTime(Monster205, 50000)
    SetChaLifeTime(Monster206, 50000)
    SetChaLifeTime(Monster207, 50000)
    SetChaLifeTime(Monster208, 50000)
    SetChaLifeTime(Monster209, 50000)
    SetChaLifeTime(Monster210, 50000)
    SetChaLifeTime(Monster211, 50000)
    SetChaLifeTime(Monster212, 50000)
    SetChaLifeTime(Monster213, 50000)
    SetChaLifeTime(Monster214, 50000)
    SetChaLifeTime(Monster215, 50000)
    SetChaLifeTime(Monster216, 50000)
    SetChaLifeTime(Monster217, 50000)
    SetChaLifeTime(Monster218, 50000)
    SetChaLifeTime(Monster219, 50000)
    SetChaLifeTime(Monster220, 50000)
    SetChaLifeTime(Monster221, 50000)
    SetChaLifeTime(Monster222, 50000)
    SetChaLifeTime(Monster223, 50000)
    SetChaLifeTime(Monster224, 50000)
    SetChaLifeTime(Monster225, 50000)
    SetChaLifeTime(Monster226, 50000)
    SetChaLifeTime(Monster227, 50000)
    SetChaLifeTime(Monster228, 50000)
    SetChaLifeTime(Monster229, 50000)
    SetChaLifeTime(Monster230, 50000)
    SetChaLifeTime(Monster231, 50000)
    SetChaLifeTime(Monster232, 50000)
    SetChaLifeTime(Monster233, 50000)
    SetChaLifeTime(Monster234, 50000)
    SetChaLifeTime(Monster235, 50000)
    SetChaLifeTime(Monster236, 50000)
    SetChaLifeTime(Monster237, 50000)
    SetChaLifeTime(Monster238, 50000)
    SetChaLifeTime(Monster239, 50000)
    SetChaLifeTime(Monster240, 50000)
    SetChaLifeTime(Monster241, 50000)
    SetChaLifeTime(Monster242, 50000)
    SetChaLifeTime(Monster243, 50000)
    SetChaLifeTime(Monster244, 50000)
    SetChaLifeTime(Monster245, 50000)
    SetChaLifeTime(Monster246, 50000)
    SetChaLifeTime(Monster247, 50000)
    SetChaLifeTime(Monster248, 50000)
    SetChaLifeTime(Monster249, 50000)
    SetChaLifeTime(Monster250, 50000)
    SetChaLifeTime(Monster251, 50000)
    SetChaLifeTime(Monster252, 50000)
    SetChaLifeTime(Monster253, 50000)
    SetChaLifeTime(Monster254, 50000)
    SetChaLifeTime(Monster255, 50000)
    SetChaLifeTime(Monster256, 50000)
    SetChaLifeTime(Monster257, 50000)
    SetChaLifeTime(Monster258, 50000)
    SetChaLifeTime(Monster259, 50000)
    SetChaLifeTime(Monster260, 50000)
    SetChaLifeTime(Monster261, 50000)
    SetChaLifeTime(Monster262, 50000)
    SetChaLifeTime(Monster263, 50000)
    SetChaLifeTime(Monster264, 50000)
    SetChaLifeTime(Monster265, 50000)
    SetChaLifeTime(Monster266, 50000)
    SetChaLifeTime(Monster267, 50000)
    SetChaLifeTime(Monster268, 50000)
    SetChaLifeTime(Monster269, 50000)
    SetChaLifeTime(Monster270, 50000)
    SetChaLifeTime(Monster271, 50000)
    SetChaLifeTime(Monster272, 50000)
    SetChaLifeTime(Monster273, 50000)
    SetChaLifeTime(Monster274, 50000)
    SetChaLifeTime(Monster275, 50000)
    SetChaLifeTime(Monster276, 50000)
    SetChaLifeTime(Monster277, 50000)
    SetChaLifeTime(Monster278, 50000)
    SetChaLifeTime(Monster279, 50000)
    SetChaLifeTime(Monster280, 50000)
    SetChaLifeTime(Monster281, 50000)
    SetChaLifeTime(Monster282, 50000)
    SetChaLifeTime(Monster283, 50000)
    SetChaLifeTime(Monster284, 50000)
    SetChaLifeTime(Monster285, 50000)
    SetChaLifeTime(Monster286, 50000)
    SetChaLifeTime(Monster287, 50000)
    SetChaLifeTime(Monster288, 50000)
    SetChaLifeTime(Monster289, 50000)
    SetChaLifeTime(Monster290, 50000)
    SetChaLifeTime(Monster291, 50000)
    SetChaLifeTime(Monster292, 50000)
    SetChaLifeTime(Monster293, 50000)
    SetChaLifeTime(Monster294, 50000)
    SetChaLifeTime(Monster295, 50000)
    SetChaLifeTime(Monster296, 50000)
    SetChaLifeTime(Monster297, 50000)
    SetChaLifeTime(Monster298, 50000)
    SetChaLifeTime(Monster299, 50000)
    SetChaLifeTime(Monster300, 50000)
    SetChaLifeTime(Monster301, 50000)
    SetChaLifeTime(Monster302, 50000)
    SetChaLifeTime(Monster303, 50000)
    SetChaLifeTime(Monster304, 50000)
    SetChaLifeTime(Monster305, 50000)
    SetChaLifeTime(Monster306, 50000)
    SetChaLifeTime(Monster307, 50000)
    SetChaLifeTime(Monster308, 50000)
    SetChaLifeTime(Monster309, 50000)
    SetChaLifeTime(Monster310, 50000)
    SetChaLifeTime(Monster311, 50000)
    SetChaLifeTime(Monster312, 50000)
    SetChaLifeTime(Monster313, 50000)
    SetChaLifeTime(Monster314, 50000)
    SetChaLifeTime(Monster315, 50000)
    SetChaLifeTime(Monster316, 50000)
    SetChaLifeTime(Monster317, 50000)
    SetChaLifeTime(Monster318, 50000)
    SetChaLifeTime(Monster319, 50000)
    SetChaLifeTime(Monster320, 50000)
    SetChaLifeTime(Monster321, 50000)
    SetChaLifeTime(Monster322, 50000)
    SetChaLifeTime(Monster323, 50000)
    SetChaLifeTime(Monster324, 50000)
    SetChaLifeTime(Monster325, 50000)
    SetChaLifeTime(Monster326, 50000)
    SetChaLifeTime(Monster327, 50000)
    SetChaLifeTime(Monster328, 50000)
    SetChaLifeTime(Monster329, 50000)
    SetChaLifeTime(Monster330, 50000)
    SetChaLifeTime(Monster331, 50000)
    SetChaLifeTime(Monster332, 50000)
    SetChaLifeTime(Monster333, 50000)
    SetChaLifeTime(Monster334, 50000)
  end
  if Every_5_minute == 8 then
		local Monster335 =CreateChaEx(1417, 21811, 3311, 205, 60,map_copy)
			SetChaLifeTime(Monster335, 6000000)
			ScrollNotice("<Chaos Argent> BD has spawned!",1)
	end
end
