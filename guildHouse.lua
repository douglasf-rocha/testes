...
------------Guild House Npc----
function Guild_House ()
	Talk( 1, "Snowy Guild Builder: Hi! i can help you to create your guild house with my magical stuff!!" )
...
-----test---
 CoolDownHouse = CoolDownHouse or {} ---CoolDownHouse cooldown
local CoolDownHouseCd = true
----test--
function enterGuildHouse(role)
	local guildID = GetChaGuildID(role)
	 local name = GetChaDefaultName(role)
	------------test--
	if CoolDownHouseCd  then
	    if CoolDownHouse[name] == nil then
			CoolDownHouse[name] = { UsedTime = os.time()}
		end
		local cooldown = CoolDownHouse[name].UsedTime - os.time()	
		 if cooldown > 0 then
			 SystemNotice(role,"Guild House cooldown wait "..cooldown.." second(s) to Teleport again!")
			 return
		 end
			CoolDownHouse[name].UsedTime = os.time() + 10
			local cd = CoolDownHouse[name].UsedTime
			local cmd = string.format([[CoolDownHouse['%s'].UsedTime = %d]],name,cd)
			Lua_All(cmd,role)
		end
		-------test---		
