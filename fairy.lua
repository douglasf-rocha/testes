...
-- CONFIGURATIONS:
----------------------------------------------------------
ELEEXP_GETRAD = 90		--> fairy exp rate
...
elf = elf or {}
	...
	elf.conf	= {
		normal	= 52,	--> max level of fairy
		upgrade	= 35,	--> max level when regular ration no longer usable
		max		= 30	--> max level of fairy with improved fruits
	}
	elf.fairyId	= {
		mordo	= 0680,
		mordojr	= 0681,
	...
	elf.LuckExpToAll = true;	--> if true, all 2nd gen receive x2 exp/drop when poss
	elf.minFairyCoin = 0;		--> lowest fairy lv to start receiving fairy coins
	elf.minToPossess = 50;		--> minimum stamina to poss
	elf.consumeStamP = false	--> if true, when poss fairy will drop stamina
	elf.minFLvToPoss = 5;		--> minimum level for fairy to use poss
	elf.disableFCoin = false		--> if true, disable Fairy producing F Coins
...
elf.ration = {}
	...
	elf.ration[6841]	= {
		stam			= 50,
		prohibit		= {elf.secondFairy}, -- for 2gen pets
		max				= elf.conf['upgrade'],
		...
		if GetItemAttr(fairy, ITEMATTR_URE) < GetItemAttr(fairy, ITEMATTR_MAXURE) then
			...
			if elf.ration[rationId].nodeduction ~= nil then
				if GetChaStateLv(role,200) >=  1 then 
					SystemNotice(role,"Happy Ration Already Active ")
					UseItemFailed(role)
				return
...
elf.handleFruits = function(role, Item, fairy)
	...
	local fairy = GetEquipItemP(role,16)
	if chaBoat ~= nil then
		SystemNotice(role, 'Cannot use fairy fruits while sailing.')
		UseItemFailed(role)
		return
	end
	local fairyId = GetItemID(fairy)
	...
	if fairy ~= GetEquipItemP(role,16) then --GetChaItem(role, 2, 1) then
		SystemNotice(role, 'Fruits can only be fed to fairies in Pet slot!')
		UseItemFailed(role)
		return
	end
    ...
    if fairyId == 0 then
        PopupNotice(role, 'Fairy Slot is empty please check it!')
        UseItemFailed(role)
        return    
    end
...
elf.levelUpRate = {}
	...
	elf.levelUpRate[20] = .70
	elf.levelUpRate[21] = .70
	elf.levelUpRate[22] = .70
	elf.levelUpRate[23] = .70
	elf.levelUpRate[24] = .70
	elf.levelUpRate[25] = .50
	elf.levelUpRate[26] = .50
	elf.levelUpRate[27] = .50
	elf.levelUpRate[28] = .50
	elf.levelUpRate[29] = .50
	elf.levelUpRate[30] = .50
...
function Skill_JLFT_BEGIN(role, sklv)			
	...
	if lv_JL < elf.minFLvToPoss then
		SystemNotice ( role , "Fairy must be at least level "..elf.minFLvToPoss.." to cast Fairy possession!" ) 
		SkillUnable(role) 
		return
	end
...
function State_JLFT_Add(Player, Lv)
	...
	if FairyType == 59 then
		...
		local Part1 = GetNum_Part1(Num_JL)
		-- cheaters
		if FairyLv > (elf.conf['max']+2) then
